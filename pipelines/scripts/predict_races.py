...
def check_warehouse_table():
    """
    Check PostgreSQL container directly to verify table exists and show data.
    """
    logger.info("Checking PostgreSQL warehouse container...")
    ...
    try:
        ...
        with DatabaseManager() as db:
            ...
            if db.table_exists(table_name):
                logger.info(f"✅ Table '{table_name}' exists in warehouse")
                ...
                # Show sample data
                if count > 0:
                    sample_query = f"SELECT race_id, race_name, predicted_winner, confidence_score FROM {table_name} LIMIT 3"
                    sample_records = db.execute_query(sample_query)
                    
                    logger.info("📋 Sample records:")
                    for record in sample_records:
                        logger.info(f"   Race {record[0]}: {record[1]} -> {record[2]} (confidence: {record[3]:.2f})")
                
                return f"Table verified: {count} records in {table_name}"
            else:
                logger.warning(f"❌ Table '{table_name}' does not exist in warehouse")
                return f"Table {table_name} not found"
                
    except Exception as e:
        logger.error(f"Error checking warehouse: {str(e)}")
        raise